...
def train_price_conv_ultra():
    ...
    BATCH_SIZE = 64 # Larger model, smaller batch
    LATENT_DIM = 32 # Increased capacity
    LEARNING_RATE = 3e-4 # More conservative for deep stack
    EPOCHS = 100
    EARLY_STOPPING_PATIENCE = 15
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
    
    print(f"Training Ultra-Complex Price VAE (5 layers, 512 filters) on {DEVICE}")
    ...
    # 3. Model
    model = PriceConvVAEUltra(input_dim=800, latent_dim=LATENT_DIM).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"[{epoch:3d}/100] Ultra Val Loss: {avg_val_loss:.4f}")
            
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            epochs_no_improve = 0
            torch.save(model.state_dict(), os.path.join(MODELS_DIR, "vae_price_conv_ultra_best.pth"))
            np.save(os.path.join(MODELS_DIR, "scaler_price_conv_ultra_mean.npy"), scaler.mean_)
            np.save(os.path.join(MODELS_DIR, "scaler_price_conv_ultra_scale.npy"), scaler.scale_)
        else:
            epochs_no_improve += 1
    ...
    print("Ultra-Complex Price VAE training complete.")

if __name__ == "__main__":
    train_price_conv_ultra()
