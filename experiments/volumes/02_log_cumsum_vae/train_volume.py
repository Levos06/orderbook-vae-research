import os
import sys
import torch
import torch.optim as optim
from torch.utils.data import DataLoader, Subset
import numpy as np
import matplotlib.pyplot as plt
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler

ROOT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)

SRC_DIR = os.path.join(ROOT_DIR, "src")
if SRC_DIR not in sys.path:
    sys.path.insert(0, SRC_DIR)

from dataset_volume import LogCumVolumeDataset
from model_price import PriceVAE, loss_function

def train_volume():
    # Configuration
    DATA_PATH = os.path.join(
        ROOT_DIR,
        "@data",
        "orderbook_data.npy",
    )
    BASE_DIR = os.path.dirname(__file__)
    MODELS_DIR = os.path.join(BASE_DIR, "models")
    PLOTS_DIR = os.path.join(BASE_DIR, "plots")
    BATCH_SIZE = 128
    LATENT_DIM = 32
    LEARNING_RATE = 1e-3
    EPOCHS = 100
    EARLY_STOPPING_PATIENCE = 10
    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
    
    print(f"Training Volume VAE with Log-CumSum strategy on {DEVICE}")
    
    # 1. Load Data
    full_dataset = LogCumVolumeDataset(DATA_PATH, method='ols')
    indices = np.arange(len(full_dataset))
    train_idx, temp_idx = train_test_split(indices, test_size=0.2, random_state=42)
    val_idx, test_idx = train_test_split(temp_idx, test_size=0.5, random_state=42)
    
    # 2. Scaling Residuals
    train_residuals = full_dataset.residuals[train_idx]
    scaler = StandardScaler()
    scaler.fit(train_residuals)
    full_dataset.residuals = scaler.transform(full_dataset.residuals)
    
    train_loader = DataLoader(Subset(full_dataset, train_idx), batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(Subset(full_dataset, val_idx), batch_size=BATCH_SIZE, shuffle=False)
    test_loader = DataLoader(Subset(full_dataset, test_idx), batch_size=BATCH_SIZE, shuffle=False)
    
    # 3. Model
    model = PriceVAE(input_dim=800, latent_dim=LATENT_DIM).to(DEVICE)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    
    os.makedirs(MODELS_DIR, exist_ok=True)
    os.makedirs(PLOTS_DIR, exist_ok=True)

    # 4. Training
    best_val_loss = float('inf')
    epochs_no_improve = 0
    
    for epoch in range(1, EPOCHS + 1):
        model.train()
        train_loss = 0
        for batch_res, _, _ in train_loader:
            batch_res = batch_res.to(DEVICE).float()
            optimizer.zero_grad()
            recon_batch, mu, log_var = model(batch_res)
            loss, _, _ = loss_function(recon_batch, batch_res, mu, log_var)
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
            
        model.eval()
        val_loss = 0
        with torch.no_grad():
            for batch_res, _, _ in val_loader:
                batch_res = batch_res.to(DEVICE).float()
                recon_batch, mu, log_var = model(batch_res)
                loss, _, _ = loss_function(recon_batch, batch_res, mu, log_var)
                val_loss += loss.item()
        
        avg_val_loss = val_loss/len(val_loader.dataset)
        if epoch % 10 == 0 or epoch == 1:
            print(f"[{epoch:3d}/100] Val Loss: {avg_val_loss:.4f}")
            
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            epochs_no_improve = 0
            torch.save(model.state_dict(), os.path.join(MODELS_DIR, "vae_volume_best.pth"))
            np.save(os.path.join(MODELS_DIR, "scaler_volume_mean.npy"), scaler.mean_)
            np.save(os.path.join(MODELS_DIR, "scaler_volume_scale.npy"), scaler.scale_)
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= EARLY_STOPPING_PATIENCE:
                print("Early stopping.")
                break
                
    # 5. Visualizations
    model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "vae_volume_best.pth")))
    model.eval()
    with torch.no_grad():
        test_subset = Subset(full_dataset, test_idx)
        res_batch, m_batch, b_batch = next(iter(DataLoader(test_subset, batch_size=3, shuffle=False)))
        
        res_batch_gpu = res_batch.to(DEVICE).float()
        recon_res_scaled, _, _ = model(res_batch_gpu)
        
        orig_res = scaler.inverse_transform(res_batch.numpy())
        recon_res = scaler.inverse_transform(recon_res_scaled.cpu().numpy())
        
        m_np, b_np = m_batch.numpy().reshape(-1, 1), b_batch.numpy().reshape(-1, 1)
        x = np.arange(800).reshape(1, -1)
        trends = m_np * x + b_np
        
        # Reconstruct Log-Cumulative Curves
        orig_log_cum = trends + orig_res
        recon_log_cum = trends + recon_res
        
        # Reconstruct Volumes: diff() then expm1()
        # To diff properly we prepend a zero (since cumsum[0] is the first vol)
        orig_log_vol = np.diff(orig_log_cum, axis=1, prepend=0)
        recon_log_vol = np.diff(recon_log_cum, axis=1, prepend=0)
        
        orig_vol = np.expm1(orig_log_vol)
        recon_vol = np.expm1(recon_log_vol)
        
        fig, axes = plt.subplots(3, 2, figsize=(16, 15))
        for i in range(3):
            # Left: Log Cumulative Reconstruction
            ax1 = axes[i, 0]
            ax1.plot(orig_log_cum[i], 'g-', alpha=0.6, label='Original')
            ax1.plot(recon_log_cum[i], 'r--', label='Reconstructed')
            ax1.set_title(f'Sample {i+1} - Log Cumulative Curve')
            ax1.legend()
            
            # Right: Raw Volume Reconstruction (Log scale for visibility)
            ax2 = axes[i, 1]
            # Plot log scale to see if the structure matches
            ax2.bar(np.arange(800), orig_log_vol[i], color='forestgreen', alpha=0.4, label='Original LogVol')
            ax2.step(np.arange(800), recon_log_vol[i], color='crimson', where='mid', label='Recon LogVol')
            ax2.set_title(f'Sample {i+1} - Log Volume Reconstruction (Log Scale)')
            ax2.legend()
            
        plt.tight_layout()
        plot_path = os.path.join(PLOTS_DIR, "volume_vae_reconstructions.png")
        plt.savefig(plot_path)
        print(f"Reconstruction plot saved to {plot_path}")

if __name__ == "__main__":
    train_volume()
