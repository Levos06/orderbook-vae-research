...
def train_volume_conv_complex():
    ...
    BATCH_SIZE = 64 # Smaller batch for more complex model
    LATENT_DIM = 32
    ...
    print(f"Training Complex Conv1D Volume VAE on {DEVICE}")
    ...
    # 3. Model
    model = ConvVAEComplex(input_dim=800, latent_dim=LATENT_DIM).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), os.path.join(MODELS_DIR, "vae_volume_conv_complex_best.pth"))
            np.save(os.path.join(MODELS_DIR, "scaler_volume_conv_complex_mean.npy"), scaler.mean_)
            np.save(os.path.join(MODELS_DIR, "scaler_volume_conv_complex_scale.npy"), scaler.scale_)
        else:
            epochs_no_improve += 1
    ...
    print("Complex Conv1D training complete.")

if __name__ == "__main__":
    train_volume_conv_complex()
