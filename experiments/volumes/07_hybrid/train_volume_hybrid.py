...
def train_volume_hybrid():
    ...
    print(f"Training Hybrid VAE on residuals of AE on {DEVICE}")
    ...
    # 3. Precompute Hybrid (VAE) residuals
    # We train the VAE on (AE_Residual - AE_Recon)
    # Actually, AE_Residual is already scaled.
    with torch.no_grad():
        ...
        # Hybrid Residual = Original Residual (Scaled) - AE Reconstruction (Scaled)
    ...
    # Update dataset object temporarily or use a wrapper
    # For simplicity, we just pass the new residuals to the loader
    ...
    # 5. Model
    model = ConvVAE(input_dim=800, latent_dim=LATENT_DIM_VAE).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(f"[{epoch:3d}/100] Hybrid VAE Val Loss: {avg_val_loss:.4f}")
            
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            epochs_no_improve = 0
            torch.save(model.state_dict(), os.path.join(MODELS_DIR, "vae_volume_hybrid_best.pth"))
            np.save(os.path.join(MODELS_DIR, "scaler_hybrid_mean.npy"), scaler_hybrid.mean_)
            np.save(os.path.join(MODELS_DIR, "scaler_hybrid_scale.npy"), scaler_hybrid.scale_)
        else:
            epochs_no_improve += 1
