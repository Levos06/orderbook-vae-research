...
class ConvVAE(nn.Module):
    def __init__(self, input_dim=800, latent_dim=32):
        ...
        self.encoder_conv = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=7, stride=2, padding=3), # (B, 16, 400)
            nn.BatchNorm1d(16),
            nn.LeakyReLU(),
            nn.Conv1d(16, 32, kernel_size=5, stride=2, padding=2), # (B, 32, 200)
            nn.BatchNorm1d(32),
            nn.LeakyReLU(),
        ...
        self.fc_mu = nn.Linear(64 * 100, latent_dim)
        self.fc_var = nn.Linear(64 * 100, latent_dim)
        
        # Decoder
        self.decoder_input = nn.Linear(latent_dim, 64 * 100)
    ...
    def decode(self, z):
        ...
        result = result.view(result.size(0), 64, 100)
